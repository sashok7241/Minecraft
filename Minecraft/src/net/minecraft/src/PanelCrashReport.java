...
public class PanelCrashReport extends Panel
{
	...
	public PanelCrashReport(CrashReport p_i3010_1_)
	{
		...
		p_i3010_1_.getCrashCause().printStackTrace(new PrintWriter(var2));
		...
		} else
		{
			...
			var6 = var6 + "      ----------------------      \n";
			var6 = var6 + "\n";
			var6 = var6 + "Minecraft has stopped running because it encountered a problem; " + p_i3010_1_.getDescription() + "\n\n";
			File var7 = p_i3010_1_.getFile();
			if(var7 == null)
			{
				p_i3010_1_.saveToFile(new File(new File(Minecraft.getMinecraftDir(), "crash-reports"), "crash-" + new SimpleDateFormat("yyyy-MM-dd_HH.mm.ss").format(new Date()) + "-client.txt"), Minecraft.getMinecraft().getLogAgent());
				var7 = p_i3010_1_.getFile();
			}
			if(var7 != null)
			{
